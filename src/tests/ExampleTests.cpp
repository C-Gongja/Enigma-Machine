...
namespace
{
	...
	Enigma *get_default_Enigma_one()
	{
		const char *rotors[] = {ROTOR_I, ROTOR_II, ROTOR_III};
		size_t rings[] = {0, 0, 0};
		size_t inits[] = {3, 3, 3};
		return new_Enigma(3, rotors, rings, inits, I_UKW_B, 0, NULL);
	}
	...
	TEST(ExampleTests, TestEnigmaOneRotorWORing)
	{
		...
		// init = 1 JEKMFLGDQVZNTOWYHXUSPAIBRC
		// one tick CJEKMFLGDQVZNTOWYHXUSPAIBR
		//  Plugboard: C->C
		//  Rotor I: C->J
		//  Reflector: J->B
		//  Reverse Rotor I: B->E
	...
	TEST(ExampleTests, TestEnigmaOneRotorWORing2)
	{
		Enigma *enigma = get_default_Enigma_one();
		char encrypt[] = "\0\0\0\0";
		encrypt_Enigma(enigma, encrypt, "ABCD");
		ASSERT_STREQ(encrypt, "TVST");
		free_Enigma(enigma);
	}
